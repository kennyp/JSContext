...
var JSContext = {
            ...
            var newContext = {
                ...
                load : function (res) {
                    if (typeof ContextCatalog[res] === 'undefined') {
                        var req = JSContext.xhr.create();
                        req.open('GET', res, false);
                        req.send(null);
                        if (JSContext.xhr.valid(req)) {
     ...
     *
     * Three was to call:
     *  JSContext.onload(contextName, environment, function)
                    ...
                    if (typeof contextList === 'object') {
                        for (context in contextList) {
                            if (contextList.hasOwnProperty(context)) {
                                window[context] = JSContext.create.apply(null, contextList[context]);
                            }
                        }
                    } else {
                        window[contextName] = JSContext.create.apply(null, env);
                    }
